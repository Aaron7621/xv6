...
Unix中的大多数资源都表示为文件，包括控制台、管道、真实文件。文件描述符层是实现这种一致性的层。此时的“文件”指的是“文件描述符”，是一个更高层的抽象概念。
...
- create()：为新inode创建一个新名称（不严格地说就是创建一个“文件”），它可以为带有O_CREATE标志的open生成一个新的普通文件，mkdir生成一个新目录，mkdev生成一个新的设备文件。
  ...
  - name不存在的话，调ialloc分配一个新的inode。根据major和minor设置inode的属性，nlink设为1。然后iupdate一下同步到磁盘
